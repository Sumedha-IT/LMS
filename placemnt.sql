...
-- 11. STUDENT PLACEMENT VIEW (Combines User Data + Placement Status)
-- =====================================================
...
-- 12. PLACEMENT STATISTICS VIEW (Reporting Data)
-- =====================================================
...
-- Insert default placement criteria
INSERT INTO `placement_criteria` (`name`, `profile_completion_percentage`, `course_completion_required`, `exam_standards_required`, `attendance_percentage`, `fees_payment_required`, `lab_test_cases_required`, `assignments_required`, `is_active`) VALUES
('Standard Placement Criteria', 90.00, 1, 1, 75.00, 1, 1, 1, 1),
('Relaxed Criteria', 80.00, 1, 1, 70.00, 0, 1, 1, 1),
('Strict Criteria', 95.00, 1, 1, 85.00, 1, 1, 1, 1);

-- Insert sample companies
INSERT INTO `companies` (`name`, `description`, `website`, `industry`, `company_size`, `contact_person`, `contact_email`, `contact_phone`, `address`) VALUES
('Google', 'Technology company specializing in internet services', 'https://google.com', 'Technology', '10000+ employees', 'John Doe', 'hr@google.com', '+91-9876543210', 'Hyderabad, India'),
('Microsoft', 'Software company developing computer software', 'https://microsoft.com', 'Technology', '5000+ employees', 'Jane Smith', 'hr@microsoft.com', '+91-9876543211', 'Bangalore, India'),
('Amazon', 'E-commerce and cloud computing company', 'https://amazon.com', 'Technology', '10000+ employees', 'Bob Johnson', 'hr@amazon.com', '+91-9876543212', 'Mumbai, India');

-- Insert sample interview questions
INSERT INTO `interview_questions` (`question`, `category`, `difficulty_level`, `domain_id`, `is_active`) VALUES
('What is object-oriented programming?', 'technical', 'easy', 1, 1),
('Explain the difference between stack and heap memory', 'technical', 'medium', 1, 1),
('Tell me about yourself', 'behavioral', 'easy', NULL, 1),
('How do you handle stress?', 'behavioral', 'medium', NULL, 1),
('What is your greatest strength?', 'behavioral', 'easy', NULL, 1);

-- =====================================================
-- SAMPLE QUERIES
-- =====================================================

-- Get all eligible students for placement
-- SELECT * FROM student_placement_view WHERE is_eligible = 1 AND is_placed = 0;

-- Get students by domain
-- SELECT * FROM student_placement_view WHERE domain_id = 1 AND is_eligible = 1;

-- Get students with incomplete profiles
-- SELECT * FROM student_placement_view WHERE profile_completion_percentage < 90.00;

-- Get placed students
-- SELECT * FROM student_placement_view WHERE is_placed = 1;

-- Get PAP students who got placed
-- SELECT * FROM student_placement_view WHERE is_pap_student = 1 AND is_placed = 1;

-- Get students with resume for bulk download
-- SELECT id, name, email, upload_resume, passport_photo_path FROM users WHERE role_id = 6 AND upload_resume IS NOT NULL AND is_active = 1;

-- Get placement statistics by domain
-- SELECT u.domain_id, COUNT(*) as total_students, SUM(CASE WHEN spe.is_eligible = 1 THEN 1 ELSE 0 END) as eligible_students, SUM(CASE WHEN spe.is_placed = 1 THEN 1 ELSE 0 END) as placed_students FROM users u LEFT JOIN student_placement_eligibility spe ON u.id = spe.user_id WHERE u.role_id = 6 AND u.is_active = 1 GROUP BY u.domain_id;

-- =====================================================
-- END OF PLACEMENT SCHEMA
-- =====================================================